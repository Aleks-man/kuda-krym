...
- Посмотреть погоду для 13 внутренних городов и посёлков Крыма: поиск по названию, точки на карте и отдельные страницы прогноза без морских показателей.
- Открыть прогноз на семь дней: температура воздуха и воды, ветер, порывы, волны
  и осадки. В верхней карточке — температура «по ощущениям», влажность и УФ-индекс, атмосферное давление и видимость.
  Для погоды сейчас используются отдельные текущие условия по модели Open-Meteo; осадки показаны за указанный интервал, вероятность дождя — в почасовом прогнозе.
...
В каталоге — 50 исходных WebP-фотографий; у всех 36 прибрежных локаций есть
...
Яндекс Метрика (счётчик `112933626`) включается только на `kudakrym.ru` и `www.kudakrym.ru`.
Переходы Next.js учитываются без перезагрузки страницы; локальные и тестовые домены не отправляют посещения.
После публикации в настройках счётчика нужно создать цели типа JavaScript-событие с идентификаторами:

- `recommendation_submit` — отправка корректно заполненной формы.
- `recommendation_results` — получены варианты пляжей.
- `recommendation_empty` — ответ получен, подходящих вариантов нет.
- `recommendation_error` — ошибка при подготовке или выполнении запроса.

Включены Вебвизор, карта кликов и отслеживание ссылок. Значения полей формы в параметры этих целей не передаются.
